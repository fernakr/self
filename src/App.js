...
function Scene({ setControlsEnabled }) {
  ...
  // on keypress of space bar, change mode
  ...
  useEffect(() => {
    ...
    shapes.forEach((shape) => {
      shape.controls.visible = currentShape === shape;   
      shape.controls.enabled = currentShape === shape;   
    });
  ...
  return (
    <group ref={groupRef}>
      { shapes.map((shape, index) => (
        <primitive onClick={ e=> { setCurrentShape(shape);}} object={shape} key={index} />
      ))}
...
function App() {
  ...
  return (
    <Canvas camera={{ position: [4.5, 4.5, 4.5], fov: 75 }}>
      <ambientLight intensity={0.9} color={0xb0bec5} />
